  ...
  <nav x-data="{ open: false }" class="bg-white shadow-sm sticky top-0 z-50 transition duration-300">
    <div class="max-w-7xl mx-auto px-6 py-4 flex justify-between items-center">
      ...
      <ul class="hidden md:flex space-x-8 text-gray-700 font-medium">
        <li>
          <a href="/" class="group relative transition-colors duration-300 {{ Request::is('/') ? 'text-amber-500 font-semibold' : 'hover:text-amber-500' }}">
            Beranda
            <span class="absolute left-0 -bottom-1 h-0.5 w-full bg-amber-500 scale-x-0 group-hover:scale-x-100 origin-left transition-transform duration-300 ease-out {{ Request::is('/') ? 'scale-x-100' : '' }}"></span>
          </a>
        </li>
        <li>
          <a href="/produk" class="group relative transition-colors duration-300 {{ Request::is('produk*') ? 'text-amber-500 font-semibold' : 'hover:text-amber-500' }}">
            Produk
            <span class="absolute left-0 -bottom-1 h-0.5 w-full bg-amber-500 scale-x-0 group-hover:scale-x-100 origin-left transition-transform duration-300 ease-out {{ Request::is('produk*') ? 'scale-x-100' : '' }}"></span>
          </a>
        </li>
        <li>
          <a href="/edukasi" class="group relative transition-colors duration-300 {{ Request::is('edukasi*') ? 'text-amber-500 font-semibold' : 'hover:text-amber-500' }}">
            Edukasi
            <span class="absolute left-0 -bottom-1 h-0.5 w-full bg-amber-500 scale-x-0 group-hover:scale-x-100 origin-left transition-transform duration-300 ease-out {{ Request::is('edukasi*') ? 'scale-x-100' : '' }}"></span>
          </a>
        </li>
        <li>
          <a href="/galeri" class="group relative transition-colors duration-300 {{ Request::is('galeri*') ? 'text-amber-500 font-semibold' : 'hover:text-amber-500' }}">
            Galeri
            <span class="absolute left-0 -bottom-1 h-0.5 w-full bg-amber-500 scale-x-0 group-hover:scale-x-100 origin-left transition-transform duration-300 ease-out {{ Request::is('galeri*') ? 'scale-x-100' : '' }}"></span>
          </a>
        </li>
      </ul>
    ...
    <div x-show="open" x-transition class="md:hidden px-6 pb-4 space-y-2 bg-white shadow-inner">
      <a href="/" class="block text-gray-700 font-medium {{ Request::is('/') ? 'text-amber-500' : '' }}">Beranda</a>
      <a href="/produk" class="block text-gray-700 font-medium {{ Request::is('produk*') ? 'text-amber-500' : '' }}">Produk</a>
      <a href="/edukasi" class="block text-gray-700 font-medium {{ Request::is('edukasi*') ? 'text-amber-500' : '' }}">Edukasi</a>
      <a href="/galeri" class="block text-gray-700 font-medium {{ Request::is('galeri*') ? 'text-amber-500' : '' }}">Galeri</a>
      <a href="https://wa.me/6281216647993" target="_blank" rel="noopener noreferrer"
